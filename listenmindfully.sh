...
function www_install {
    ...
    # TODO use git to clone the application source and place in /var/www
    git clone https://github.com/eric-isakson/listenmindfully.org.git /var/www
    mkdir /var/www
cat <<EOT >/var/www/app.js
...
// In this case, it is app.js a simple node http app
...
function stopWorker(worker) {
  ...
  // Ensure we do not stay up just for this setTimeout
  killTimer.unref();
}

// Tell the next worker queued to restart to disconnect
// This will allow the process to finish work
